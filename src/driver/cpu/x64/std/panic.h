 ...
 */
struct panic_data {
    const std::uint16_t instruction;  ///< Bytes of instruction that caused panic
    const panic_type type;            ///< Validates as actual PANIC() and its cause
    const char * filename;            ///< Source filename of PANIC() invocation
    const std::uint16_t lineNum;      ///< Line number of PANIC() invocation
    const char8_t * msg;              ///< String message describing PANIC()
...
do {                                                       \
    asm volatile("ud2        # panic_data::instruction\n"  \
                 ...
                 : /* No outputs */                        \
                 : "i" ((std::uint16_t)type), "i" (__FILE__),   \
                   "i" (__LINE__), "i" (msg)               \
                 : /* No clobbers */);                     \
} while(0)
